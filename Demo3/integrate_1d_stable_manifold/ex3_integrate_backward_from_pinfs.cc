/*
 * ex3_integrate_backward_from_p_inf_s.cc
 *
 * Validation code of connecting orbit using integration backward
 * from stable manifold attached p_{inf,s}
 * to the equilibrium p_b of
 *
 ...
 *
 *
 * written  Oct. 13, 2020 Akitoshi Takayasu (with kv-0.4.48)
 *
 */
...
int main()
{
    ...
    pb.resize(n);
    pb(0) = 0.7328506362011802;
    pb(1) = 0.5370700549804747;
    ...
    // std::cout << "Norm(x)" << norm_2(x) << std::endl;
...
//     if (1.<6*pow(abs(p(0)),2)) {
//       kappa = 6*pow(abs(p(0)),2);
//     } else {
//       kappa = 1.;
//     }
//     std::cout << "T_max-T_n: " << mag((
//       4*pow((pow(p(1),2)+4*pow(p(0),6))*Lx/c1,0.5)
//       +kappa*Lx
//       +8./3*abs(p(0))*pow(c1,0.5)*pow(Lx,1.5)
